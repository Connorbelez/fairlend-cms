...
| Investor route selector language | Investor copy now says `Private-mortgage investing with the administration handled.` and avoids `real estate-backed investing`. | `src/components/FairlendRouteSelector/route-data.tsx:34` |
...
| Replace investor card line with `Private-mortgage investing with the administration handled`; avoid `real estate-backed investing`. Source p007. | Resolved in implementation pass; see Implementation Resolution. | See Implementation Resolution. |
