...
function getPostAuthor(post: FairlendJournalPost): string {
  return post.populatedAuthors?.find((author) => author.name)?.name || 'FairLend editorial desk'
}
...
function FeaturedStory({ post }: { post: FairlendJournalPost }) {
  ...
  return (
    <Link
      className="group grid min-h-[31rem] border-y border-[#08090a] bg-[#fffdf9] text-[#08090a] md:grid-cols-[minmax(0,1.22fr)_minmax(21rem,0.78fr)]"
        ...
        {archivePosts.length > 0 ? (
          <section aria-labelledby="journal-archive-heading" className="py-16 sm:py-20 lg:py-24">
            <div className="mb-10 flex flex-col gap-5 border-b border-[#deded8] pb-6 sm:flex-row sm:items-end sm:justify-between">
              ...
              <p className="text-sm font-bold text-[#494944]">
                {archivePosts.length} {archivePosts.length === 1 ? 'field note' : 'field notes'}
              </p>
            </div>
