...
import { IconEdit, IconTrash, IconCalendarPlus, IconVideo as IconJoin } from '@tabler/icons-react'

const PlatformIcon = ({ platform }: { platform: string }) => {
  switch (platform) {
    case 'meet':
      return <IconVideoFilled className="size-7 text-blue-500" />
    case 'zoom':
      return <IconVideoPlusFilled className="size-7 text-blue-600" />
    case 'discord':
      return <IconBrandDiscordFilled className="size-7 text-indigo-500" />
    case 'slack':
      return <IconBrandSlack className="size-7 text-[#4A154B]" />
    case 'google':
      return <IconBrandGoogleFilled className="size-7 text-red-500" />
    default:
      return <IconVideo className="size-7" />
  }
}
  ...
  <Card className="border-[1.3px] border-neutral-200/80 bg-neutral-100/60 dark:bg-neutral-800/50 dark:border-neutral-800 shadow-none px-2 pt-1.5 pb-2.5 gap-2 transition-colors duration-200 hover:bg-neutral-200/50 dark:hover:bg-neutral-800/80 cursor-pointer group outline-none">
    <CardHeader className="p-0! flex flex-row items-center justify-between space-y-0">
      <div className="flex items-center gap-1.5 overflow-hidden">
        ...
        <CardTitle className="text-sm font-semibold text-neutral-600 dark:text-neutral-300 truncate tracking-tight">
          {meeting.title}
        </CardTitle>
      </div>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuLabel className="text-xs font-semibold text-neutral-500 dark:text-neutral-400">
            Card Actions
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
    ...
    <CardContent className="p-0 px-0.5 space-y-3">
      <div className="space-y-1">
        <p className="text-[13px] tracking-tight font-medium text-neutral-400 dark:text-neutral-500">
          Date
        </p>
      ...
      <div className="space-y-1">
        <p className="text-[13px] font-medium text-neutral-400 dark:text-neutral-500 tracking-tight">
          Meeting Host
        </p>
        <div className="flex items-center gap-1.5 text-[12px] text-neutral-600 dark:text-neutral-400 font-medium tracking-tight">
          <Avatar className="size-4">
            ...
            <AvatarFallback className="text-[9px] bg-neutral-200 dark:bg-neutral-800">
              {meeting.host.name.charAt(0)}
            </AvatarFallback>
          </Avatar>
          {meeting.host.name}{' '}
          {meeting.host.isMe && (
            <span className="text-neutral-400 dark:text-neutral-500 font-normal">(You)</span>
          )}
        </div>
      </div>

      <div className="space-y-1">
        <p className="text-[12px] font-medium text-neutral-400 dark:text-neutral-500 tracking-tight">
          Meeting Duration
        </p>
