...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-medium border transition-colors duration-200 cursor-default ${
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Enter the details for the new invoice here.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    ...
                    className="col-span-3 focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-neutral-400 dark:focus-visible:border-neutral-600 outline-none shadow-none"
                  />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
          ...
          <Dialog>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>Anyone with the link can view this invoice.</DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="status-accepted" className="text-xs">
                      Accepted
                    </Label>
                  </div>
                  <div className="flex items-center gap-2">
                    <Checkbox id="status-review" />
                    <Label htmlFor="status-review" className="text-xs">
                      Under Review
                    </Label>
                  </div>
                  <div className="flex items-center gap-2">
                    <Checkbox id="status-rejected" />
                    <Label htmlFor="status-rejected" className="text-xs">
                      Rejected
                    </Label>
                  </div>
                </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border bg-neutral-950/5 dark:bg-neutral-950">
              ...
              <TableHead className="text-[10px] sm:text-xs text-muted-foreground font-medium border-r border-border min-w-[150px]">
                Client Name
              </TableHead>
              <TableHead className="text-[10px] sm:text-xs text-muted-foreground font-medium border-r border-border">
                Deal Value
              </TableHead>
              <TableHead className="text-[10px] sm:text-xs text-muted-foreground font-medium border-r border-border">
                Business Report
              ...
              <TableHead className="text-[10px] sm:text-xs text-muted-foreground font-medium">
                Category
              </TableHead>
            </TableRow>
          </TableHeader>
          ...
          <TableBody className="[&_tr:last-child]:border-b">
            {invoiceData.map((row) => (
              <TableRow
                key={row.id}
                className={`border-border hover:bg-muted/20 border-b transition-colors duration-200 ${
      ...
      <div className="flex flex-col md:flex-row items-center justify-between px-4 py-3 border-t border-border gap-4 md:gap-2 bg-background">
            ...
            {[1, 2, 3, 4, 5].map((page) => (
              <PaginationItem key={page} className={page > 3 ? 'hidden sm:block' : ''}>
                <PaginationLink
                  ...
                  className={`size-8 sm:size-7 text-[10px] sm:text-xs rounded transition-colors duration-200 ${
                    currentPage === page
        ...
        <div className="flex items-center gap-2 order-3">
          ...
          <Select value={String(rowsPerPage)} onValueChange={(v) => setRowsPerPage(Number(v))}>
            <SelectTrigger className="h-6 sm:h-5 w-fit text-[10px] sm:text-xs px-2 sm:px-2.5 bg-muted border-none text-muted-foreground">
              <SelectValue />
            </SelectTrigger>
