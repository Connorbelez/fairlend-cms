    ...
    default:
      return (
        <div className="bg-zinc-100/50 dark:bg-zinc-500/10 border-[1.5px] border-zinc-200/60 dark:border-zinc-500/20 p-1 rounded-full shrink-0 transition-colors duration-200 hover:bg-zinc-200/50 dark:hover:bg-zinc-500/20 cursor-default">
          {type === 'mail' ? (
            <IconMail className="size-3.5 text-zinc-500 dark:text-zinc-400" />
          ) : (
            <IconFileText className="size-3.5 text-zinc-500 dark:text-zinc-400" />
          )}
        </div>
      )
  }
        ...
        <CardContent className="p-0 flex flex-col pointer-events-none select-none">
          <div className="p-4 flex flex-col gap-2.5">
            <div className="flex items-start justify-between gap-2">
              <h4 className="leading-tight font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 group-hover:text-zinc-950 dark:group-hover:text-white transition-colors text-sm">
                {card.title}
              </h4>
          ...
          <div
            className={`px-4 py-2.5 flex items-center justify-between gap-3 border-t border-neutral-200 dark:border-zinc-800 ${StatusStyles.footerBg}`}
          >
            <Popover>
              <PopoverTrigger asChild>
                <div className="flex items-center gap-2 min-w-0 pointer-events-auto cursor-pointer hover:z-10 group/avatar">
                  <Avatar className="size-4.5 shadow-xs shrink-0 transition-transform duration-200">
                    <AvatarFallback
                      className={`${card.avatarColor} text-[6px] text-white font-bold`}
                    >
                      {card.user
                        .split(' ')
                        .map((n: string) => n[0])
                  ...
                  <span className="text-xs font-semibold text-zinc-600 dark:text-zinc-400 group-hover/avatar:text-zinc-900 dark:group-hover/avatar:text-white truncate transition-colors">
                    {card.user}
                  </span>
                </div>
              </PopoverTrigger>
              <PopoverContent
                align="start"
                className="w-56 p-1.5 shadow-lg border-neutral-200 dark:border-zinc-800 pointer-events-auto"
              >
                <div className="flex items-center gap-2.5 px-2 py-2 mb-1 border-b border-neutral-100 dark:border-zinc-800/50 pb-3">
                  <Avatar className="size-9 rounded-full">
                    <AvatarImage src="" />
                    <AvatarFallback
                      className={`${card.avatarColor} text-white font-medium text-xs rounded-full`}
                    >
                      {card.user
                        .split(' ')
                        .map((n: string) => n[0])
            ...
            <div className="flex items-center gap-3 text-xs font-medium tracking-tight shrink-0">
              <div className={`flex items-center gap-1 shrink-0 ${StatusStyles.textColor}`}>
                <IconCalendarPlus
                  strokeWidth={1.7}
                  className={`size-4 shrink-0 ${StatusStyles.iconColor}`}
                />
