    ...
    <div className="flex flex-col gap-4.5 w-[85vw] sm:w-[320px] lg:w-auto shrink-0 snap-center lg:snap-align-none h-full min-h-0">
      <div className="flex justify-between items-center px-2.5 pointer-events-none select-none">
        <div className="flex flex-col gap-0.5 min-w-0">
          <h3 className="font-semibold tracking-tight text-[15px] text-zinc-900 dark:text-zinc-50 truncate">
            {column.title}
          </h3>
          <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[12px] text-muted-foreground font-medium">
            <span className="whitespace-nowrap">
              {column.cards.length} {column.id === 'closed' ? 'Deals' : 'Leads'}
            </span>
            <span className="hidden sm:inline-block">•</span>
            <span className="whitespace-nowrap">
              {column.total}{' '}
              <span className="text-neutral-300 dark:text-neutral-600 font-normal">Total</span>
            </span>
          </div>
        </div>
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className="size-8 text-muted-foreground hover:text-foreground dark:hover:text-zinc-100 dark:hover:bg-zinc-800 transition-colors duration-200 pointer-events-auto outline-none focus-visible:ring-0"
            >
          ...
          <PopoverContent
            align="end"
            className="w-48 p-1.5 shadow-lg border-neutral-200 dark:border-zinc-800 pointer-events-auto"
          >
