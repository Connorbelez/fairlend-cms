...
export const SiteHeader = ({ currentView }: { currentView?: string }) => {
  ...
  const filteredCompanies = useMemo(() => {
    ...
    if (!q) return []
    return companiesData.filter((c) => c.name.toLowerCase().includes(q)).slice(0, 4)
  }, [query])
  ...
  return (
    <header className="flex h-16 justify-between border-neutral-200 pr-5 dark:border-none">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <DialogContent className="top-[14%] translate-y-0 gap-0 overflow-hidden p-0 sm:max-w-lg">
