...
function ActivityTrend() {
  const heights = useMemo(
    () => [35, 62, 48, 80, 55, 92, 44, 68, 76, 51, 84, 60].map((height) => `${height}%`),
    [],
  )
  ...
  return (
    <div className="flex h-3 items-end gap-0.5">
      {heights.map((height, i) => (
        <div key={i} className="w-1 bg-green-500" style={{ height, minHeight: '2px' }} />
      ))}
    </div>
  )
}
              ...
              >
                ...
                <TableCell className="px-4">
                  <div className="flex items-center gap-2">
                    <Avatar className="size-5">
                      ...
                      <AvatarFallback>{company.accountOwner.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                ...
                <TableCell className="px-4 text-neutral-600 dark:text-neutral-400">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">{company.lastInteraction.date}</span>
                    <Separator
