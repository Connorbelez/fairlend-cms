...
export function TeamOnDuty({ className, ...props }: ComponentProps<typeof Card>) {
  const [teammates, setTeammates] = useState<Teammate[]>(() => [...INITIAL_TEAMMATES])

  function pullNextConversation(id: string) {
    setTeammates((prev) =>
      prev.map((t) => (t.id === id ? { ...t, open: Math.max(0, t.open - 1) } : t)),
    )
  }
            ...
            <li className="flex items-center gap-2 p-3 first:pt-0 last:pb-0 sm:gap-3" key={t.id}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button aria-label={`Actions for ${t.name}`} size="icon" variant="ghost">
                    <EllipsisIcon />
                  </Button>
                </DropdownMenuTrigger>
