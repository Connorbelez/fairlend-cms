...
import { formatChartAxisTick, formatChartTooltipDate, parseIsoCalendarDate } from './formater'
...
export function ConversationVolumeChart({ className, ...props }: ComponentProps<typeof Card>) {
    ...
    >
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 space-y-2">
          ...
          <CardDescription>New threads per day for the selected window.</CardDescription>
        </div>
        <Select
          onValueChange={(v) => {
            const n = Number(v)
