...
function statusVariant(state: Conversation['state']): ComponentProps<typeof Badge>['variant'] {
  if (state === 'waiting') {
    return 'destructive'
  }
  if (state === 'snoozed') {
...
export function RecentConversations({ className, ...props }: ComponentProps<typeof Card>) {
  return (
            ...
            {rows.map((r) => {
              return (
                <TableRow className="h-14 hover:bg-transparent" key={`${r.customer}-${r.subject}`}>
                  <TableCell className="max-w-36 truncate pl-6 font-medium">{r.customer}</TableCell>
                  <TableCell className="hidden max-w-32 sm:table-cell">
                    <span className="line-clamp-1 text-muted-foreground text-sm">{r.subject}</span>
                  </TableCell>
                  <TableCell>
