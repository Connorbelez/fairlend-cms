...
const testimonialQuoteVariants = cva('grow text-pretty text-foreground', {
  variants: {
    variant: {
      ...
      fairlend:
        'relative z-10 m-0 px-6 pt-[46px] pb-5 text-[18px] leading-[1.34] font-extrabold tracking-normal text-[#092b35] sm:text-[19px]',
    },
  },
  defaultVariants: {
    variant: 'default',
  ...
  'grid grid-cols-[auto_1fr] grid-rows-2 items-center gap-x-3.5',
  {
    variants: {
      variant: {
        ...
        fairlend:
          'relative z-10 mt-auto mx-4 mb-4 rounded-[7px] border border-white/70 bg-[rgb(255_255_255/34%)] px-4 py-3 shadow-[inset_0_1px_0_rgb(255_255_255/80%)] backdrop-blur-[2px]',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const testimonialAvatarVariants = cva('relative row-span-2 shrink-0', {
  variants: {
    variant: {
      default: 'size-8',
      fairlend:
        'grid size-12 place-items-center rounded-full bg-[#062c2f] text-[13px] font-extrabold text-[#fffaf4] shadow-[0_9px_18px_rgb(6_44_47/20%),inset_0_1px_0_rgb(255_255_255/18%)]',
    },
  },
  defaultVariants: {
    variant: 'default',
  ...
  'flex items-center gap-1.5 font-semibold text-foreground',
  {
    variants: {
      variant: {
        ...
        fairlend: 'text-[15px] leading-5 font-extrabold text-[var(--fairlend-ink)]',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const testimonialAuthorTaglineVariants = cva('text-balance text-muted-foreground', {
  variants: {
    variant: {
      ...
      fairlend: 'text-[12px] leading-4 font-bold text-[var(--fairlend-muted)]',
    },
  },
  defaultVariants: {
    variant: 'default',
