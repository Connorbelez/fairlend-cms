...
type MapMarkerProps = {
  ...
  /** Accessible label used when the marker has an onClick action */
  ariaLabel?: string
...
function MapMarker({
  ...
  useEffect(() => {
    ...
    if (onClick) {
      ...
      markerElement.setAttribute('aria-label', ariaLabel ?? 'Map marker')
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.key === 'Enter' || e.key === ' ') && callbacksRef.current.onClick) {
        e.preventDefault()
        markerElement.click()
      }
    }
