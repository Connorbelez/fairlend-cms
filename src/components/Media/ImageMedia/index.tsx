...
const buildLegacyMediaAlt = (src: StaticImageData | string): string => {
  const source = typeof src === 'string' ? src : src.src
  const filename = source.split('/').pop()?.split('?')[0]?.replace(/\.[^.]+$/, '') ?? ''
  const description = filename
    .replace(/[-_]+/g, ' ')
    .replace(/\b(?:asset|image|img|optimized|webp|png|jpg|jpeg|avif)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim()

  if (!description || /^[a-f0-9]{8,}$/i.test(description)) {
    return 'FairLend real estate financing image'
  }

  const brandedDescription = description.replace(/\bfairlend\b/gi, 'FairLend')
  return brandedDescription.charAt(0).toUpperCase() + brandedDescription.slice(1)
}
 ...
 *      <Image loader={imageLoader} src="/media/hero.jpg" width={1200} height={600} alt="Toronto residential construction financing project" />
 *   C) Skip optimization:
 *      <Image unoptimized src="https://cdn.example.com/hero.jpg" width={1200} height={600} alt="Toronto residential construction financing project" />
 *
...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  if (!src && resource && typeof resource === 'object') {
    ...
    width = fullWidth!
    height = fullHeight!
    if (alt === undefined) alt = altFromResource?.trim() || undefined
  ...
  return (
    <picture className={cn(pictureClassName)}>
      <NextImage
        alt={alt !== undefined ? alt : buildLegacyMediaAlt(src)}
