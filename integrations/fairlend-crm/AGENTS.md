...
## FairLend production rules

- This package is the source of truth for FairLend's Twenty metadata.
- Keep every deployed `universalIdentifier` stable. Never regenerate existing identifiers.
- Preserve the website-to-Twenty field contract documented in `../../docs/twenty-crm.md`.
- Run `yarn typecheck`, `yarn lint`, `yarn test:unit`, and `yarn twenty plan` after schema changes.
- Never run `yarn twenty apply` unless the user explicitly authorizes the production mutation.
- Never use `--force` unless the user explicitly approves the destructive plan and data migration.
- Treat object/field deletion, type changes, relation changes, uniqueness changes, and nullability changes as destructive.
- Keep schema changes separate from CRM record-data operations.
